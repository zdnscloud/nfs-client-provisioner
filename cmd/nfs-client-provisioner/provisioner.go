...
func getNfsCapacity() (*resource.Quantity, error) {
	var size float64
	...
	for i := 1; i < len(outputs); i++ {
		...
		i, err := strconv.ParseFloat(line[len(line)-5], 64)
		if err != nil {
			return nil, err
		}
		size = i
	}
	s := int64(math.Floor(size / 1024 / 1024))
	return resource.NewQuantity(s*1024*1024*1024, resource.BinarySI), nil
}
